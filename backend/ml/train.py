...
SEQ_LEN          = 24   # use last 24 hours to predict
...
def train_for_station(station: Station, aqi_data: np.ndarray):
    print(f"\n── Training: {station.name} ({len(aqi_data)} readings) ──")
    ...
    # reshape X for LSTM: (samples, seq_len, 1)
    ...
    # compute RMSE on val set (in original AQI scale)
    model.eval()
    all_preds, all_true = [], []
    with torch.no_grad():
        for xb, yb in val_dl:
...
def train_all():
    ...
    try:
        ...
        for station in stations:
            ...
            result = train_for_station(station, aqi_data)
            if result[0] is None:
                continue

            model, scaler, rmse = result

            # save model + scaler
            model_path  = os.path.join(MODEL_DIR, f"station_{station.id}_model.pt")
            scaler_path = os.path.join(MODEL_DIR, f"station_{station.id}_scaler.pkl")
